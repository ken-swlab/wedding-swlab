...
import { categoryDef, invitationDef } from "@/config/roster";
import { normalizeName, parseRoster } from "@/lib/roster";
import type { GuestRow } from "@/types/admin";

const SAMPLE = `前川 太郎, まえかわ たろう, 新郎友人, 送付済
...
export function RosterImportDialog({
  ...
  existing: GuestRow[];
  onClose: () => void;
  onImport: (rows: { displayName: string; kana: string; category: string; invitationStatus: string }[]) => Promise<void>;
}) {
  ...
  async function run() {
    ...
    try {
      await onImport(target.map(({ displayName, kana, category, invitationStatus }) => ({ displayName, kana, category, invitationStatus })));
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white">
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-stone-100 p-4">
          <div>
            <h2 className="font-serif text-lg text-stone-900">名簿を一括追加</h2>
            <p className="mt-1 text-xs leading-relaxed text-stone-500">1行に1人。カンマ・タブ区切り（Excel からの貼り付けも可）。<br /><span className="text-stone-400">名前以外の列は順不同です。「新郎友人」「送付済」などの語を自動で判別し、残りをふりがなとして扱います。</span></p>
          </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 md:grid-cols-2">
          ...
          <div className="min-h-0">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-xs font-medium text-stone-600">プレビュー <span className="ml-1.5 tabular-nums text-stone-400">{parsed.length} 行</span></p>
              {dupCount > 0 && <label className="flex items-center gap-1.5 text-[11px] text-amber-700"><input type="checkbox" checked={skipDuplicates} onChange={(e) => setSkipDuplicates(e.target.checked)} />重複 {dupCount} 件を除く</label>}
            </div>
            <div className="max-h-[46vh] overflow-y-auto rounded-xl border border-stone-200">
              {parsed.length === 0 ? <p className="p-8 text-center text-xs text-stone-400">左に貼り付けるとここに結果が出ます</p> : (
                <table className="w-full text-xs">
                  <tbody>
                    {parsed.map((r) => (
                      <tr key={r.line} className={`border-b border-stone-100 last:border-0 ${r.duplicate && skipDuplicates ? "opacity-35" : ""}`}>
                        <td className="px-2 py-1.5"><span className="block font-medium text-stone-800">{r.displayName}</span>{r.kana && <span className="block text-[10px] text-stone-400">{r.kana}</span>}</td>
                        <td className="px-1 py-1.5"><span className={`rounded-full px-1.5 py-0.5 text-[10px] ring-1 ring-inset ${categoryDef(r.category).className}`}>{categoryDef(r.category).label}</span></td>
                        <td className="px-1 py-1.5"><span className={`rounded-full px-1.5 py-0.5 text-[10px] ${invitationDef(r.invitationStatus).className}`}>{invitationDef(r.invitationStatus).label}</span></td>
                        <td className="px-2 py-1.5 text-right">{r.duplicate && <span className="text-[10px] text-amber-600">既存</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
